...
const initialRandomImage = imagesList[0].imageUrl
...
class App extends Component {
  ...
  tick = () => {
    ...
    if (timer > 1) {
      ...
      this.setState({isGameRunning: false, timer: 0})
    }
  }
  ...
  checkSelectedImage = id => {
    const {randomImage, isGameRunning} = this.state
    if (randomImage === id) {
      const newId = Math.ceil(Math.random() * 29)
      const newRandomImage = imagesList[newId].imageUrl
  ...
  playAgain = () => {
    this.timerId = setInterval(this.tick, 1000)
    this.setState({isGameRunning: true, score: 0, timer: 60})
  }
  ...
  render() {
    const {initialList, activeTab, score, isGameRunning, timer} = this.state
      ...
      <div className="app-container">
        <ul className="nav-bar">
          ...
          <li className="score-and-timer">
            <p className="score">
              Score: <span className="score-and-time">{score}</span>
            </p>
            <div className="timer-container">
              ...
              />
              <p className="score-and-time">{timer} sec</p>
            </div>
          </li>
        </ul>
              ...
              <ul className="thumbnails-container">
                {filteredList.map(eachThumbnail => (
                  <ThumbnailItem
                    ...
                    checkSelectedImage={this.checkSelectedImage}
